...
class ResponderClient:
    ...
    def create_list(self, data):
        res = requests.post(ResponderClient.lists_url, data=data, headers=self.auth_header())
        return json.loads(res.content)
    
    def update_list(self, list_id, data):
        url = ResponderClient.lists_url + list_id
        res = requests.put(url, data=data, headers=self.auth_header)
        return res.loads(res.content)
